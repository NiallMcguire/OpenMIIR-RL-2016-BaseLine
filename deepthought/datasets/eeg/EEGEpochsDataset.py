...
xrange=range
# TODO optional import when needed
#import librosa
...
def as_onehot(targets, max_labels):
    # adapted from pylearn2.format.target_format.OneHotFormatter
    one_hot = np.zeros(targets.shape + (max_labels,), dtype=targets.dtype)
    shape = (np.prod(one_hot.shape[:-1]), one_hot.shape[-1])
    one_hot.reshape(shape)[np.arange(shape[0]), targets.flatten()] = 1
    return one_hot

class EEGEpochsDataset(object):
    """
    TODO classdocs
    """
    class Like(object):
    ...
    def __init__(self,
        ...
        if use_targets:
            ...
            if label_map is None:
                self.targets = as_onehot(labels, max(labels) + 1)
            else:
                self.targets =  as_onehot(labels, max(label_map.values()) + 1)                        
    ...
    #### TODO: code below needs to be checked !!!
